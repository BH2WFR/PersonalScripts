...
| Script | Description | Requirements |
|--------|-------------|--------------|
| `run-script.py` | Unified script launcher. Loads settings from `launcher-config.yaml` with an optional personal patch; supports platform-, architecture-, and Linux-GUI-aware filtering, a configurable Test group, recursive bare-name lookup, direct numeric selection, ambiguity selection, additional directories, `@test:`, and `@N:` targeting. Python targets use a leading `--env=<name>`, then `ZL_CONDA_ENV`, then `base`; an environment prefix can also precede an interactive selector. Normal startup avoids external version probes; use leading `--env-info` to include Conda, PowerShell, and Bash versions | Cross-platform. `Python 3.13+`, `PyYAML`, `pathspec`; selected Conda environment must contain Python |
