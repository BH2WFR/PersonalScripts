...
""")
    ...
    SUBTITLE_LANGS_DEFAULT = "en.*|zh-.*|ko.*|ja.*"
    ...
    while True:
        ...
        PART_INPUT = Input.prompt(
            f"{FLYellow}Parts, e.g. `1` `1,2` or range `[START]:[STOP][:STEP]` (negative = from end), example: `1:3,7,-5::2` (default: ALL): {CRst}",
            default="",
            transform=str.upper,
        )

        #* 字幕
        IS_SUBTITLE_INPUT = Input.prompt(
            f"{FLYellow}Download subtitles? (y/n, default y): {CRst}",
            default="y",
            transform=str.lower,
        )
        ...
        #* 字幕语言
        SUBTITLE_LANGS = Input.prompt(
            f"{FLYellow}Enter subtitle languages, split by comma (default: {SUBTITLE_LANGS_DEFAULT}): {CRst}",
            default=SUBTITLE_LANGS_DEFAULT,
        )

        #* cookies
        COOKIE_PATH = Input.prompt(
            f"{FLYellow}Enter path to cookies file (export cookies to txt by this chrome extension https://chromewebstore.google.com/detail/get-cookiestxt-locally/cclelndahbckbenkjhflpdbgdldlbecc) (default: none): {CRst}",
            default="",
        )
